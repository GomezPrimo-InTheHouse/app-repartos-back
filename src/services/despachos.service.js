...
// async function crear({ propietarioId, cliente_id, items, envases_devueltos_sueltos, notas, createdBy }) {
//   const tieneItems = Array.isArray(items) && items.length > 0;
//   const tieneSueltos = Array.isArray(envases_devueltos_sueltos) && envases_devueltos_sueltos.length > 0;

//   if (!tieneItems && !tieneSueltos) {
//     throw Object.assign(
//       new Error('El despacho debe tener al menos un producto o una devolución de envases'),
//       { status: 400 }
//     );
//   }
...
//     // Validar devoluciones de envases sueltas (sin producto comprado en este despacho)
//     const sueltosProcesados = [];
//     if (tieneSueltos) {
//       for (const suelto of envases_devueltos_sueltos) {
//         if (!(Number(suelto.cantidad) > 0)) {
//           throw Object.assign(new Error('La cantidad de envases devueltos debe ser mayor a 0'), { status: 400 });
//         }

//         const { rows: productoRows } = await client.query(
//           'SELECT * FROM productos WHERE id = $1 AND propietario_id = $2',
//           [suelto.producto_id, propietarioId]
//         );
//         const producto = productoRows[0];

//         if (!producto) {
//           throw Object.assign(new Error(`Producto ${suelto.producto_id} no encontrado`), { status: 404 });
//         }
...
//         sueltosProcesados.push({ producto_id: producto.id, cantidad: Number(suelto.cantidad) });
//       }
//     }
...
//     const tipo = tieneItems ? 'venta' : 'solo_devolucion';

//     const { rows: despachoRows } = await client.query(
//       `INSERT INTO despachos (propietario_id, cliente_id, total, tipo, alerta_credito_al_momento, notas, created_by)
//        VALUES ($1, $2, $3, $4, $5, $6, $7)
//        RETURNING *`,
//       [propietarioId, cliente_id, total, tipo, alertaCredito, notas, createdBy]
//     );
...
//     // Movimientos de envase "sueltos": devolución pura, delta siempre negativo
//     for (const suelto of sueltosProcesados) {
//       await client.query(
//         `INSERT INTO movimientos_envase (propietario_id, cliente_id, producto_id, despacho_id, delta, tipo, created_by)
//          VALUES ($1, $2, $3, $4, $5, 'despacho', $6)`,
//         [propietarioId, cliente_id, suelto.producto_id, despacho.id, -suelto.cantidad, createdBy]
//       );
//     }

//     await client.query('COMMIT');

//     return { ...despacho, items: itemsProcesados, envases_devueltos_sueltos: sueltosProcesados };
//   } catch (err) {
...
//        EXISTS (
//          SELECT 1 FROM movimientos_envase me
//          LEFT JOIN despacho_items di ON di.despacho_id = me.despacho_id AND di.producto_id = me.producto_id
//          WHERE me.despacho_id = d.id AND me.tipo = 'despacho' AND me.motivo IS NULL
//            AND (di.id IS NULL OR me.delta < di.cantidad)
//        ) AS tiene_devolucion_envases,
//        EXISTS (
//          SELECT 1 FROM (
//            SELECT producto_id, SUM(delta) AS saldo
//            FROM movimientos_envase
//            WHERE cliente_id = d.cliente_id AND propietario_id = d.propietario_id
//            GROUP BY producto_id
//          ) s
//          WHERE s.saldo > 0
//        ) AS cliente_tiene_envases
//      FROM despachos d
...
//     if (!item.maneja_envase) {
//       items.push({ ...item, envases_devueltos: null, envases_saldo_anterior: null, envases_saldo_posterior: null });
//       continue;
//     }
...
//   // Movimientos de envase "sueltos" (sin item/producto comprado asociado en este despacho)
//   const { rows: movimientosSueltos } = await db.query(
//     `SELECT me.producto_id, p.nombre AS producto_nombre, me.delta, me.secuencia
//      FROM movimientos_envase me
//      JOIN productos p ON p.id = me.producto_id
//      WHERE me.despacho_id = $1 AND me.tipo = 'despacho' AND me.motivo IS NULL
//        AND me.producto_id NOT IN (SELECT producto_id FROM despacho_items WHERE despacho_id = $1)`,
//     [id]
//   );

//   const envasesDevueltosSueltos = [];
//   for (const mov of movimientosSueltos) {
//     const { rows: saldoAnteriorRows } = await db.query(
//       `SELECT COALESCE(SUM(delta), 0) AS saldo
//        FROM movimientos_envase
//        WHERE cliente_id = $1 AND producto_id = $2 AND secuencia < $3`,
//       [despacho.cliente_id, mov.producto_id, mov.secuencia]
//     );
//     const saldoAnterior = Number(saldoAnteriorRows[0].saldo);
//     envasesDevueltosSueltos.push({
//       producto_id: mov.producto_id,
//       producto_nombre: mov.producto_nombre,
//       cantidad: -mov.delta,
//       envases_saldo_anterior: saldoAnterior,
//       envases_saldo_posterior: saldoAnterior + Number(mov.delta),
//     });
//   }

//   return { ...despacho, items, envases_devueltos_sueltos: envasesDevueltosSueltos };
// }
  ...
  try {
    ...
    if (tieneItems) {
      for (const item of items) {
        ...
        const precioAplicado = resolverPrecioLinea(cliente, producto);
        const subtotal = Number(item.cantidad) * precioAplicado;
        total += subtotal;
