...
const { CLAVES_VALIDAS } = require('../config/modulos');

function validarPermisos(permisos) {
  if (permisos === undefined) return [];
  if (!Array.isArray(permisos)) {
    throw Object.assign(new Error('permisos debe ser un array de strings'), { status: 400 });
  }
  const invalidos = permisos.filter((p) => !CLAVES_VALIDAS.includes(p));
  if (invalidos.length > 0) {
    throw Object.assign(
      new Error(`Módulo(s) inválido(s): ${invalidos.join(', ')}`),
      { status: 400 }
    );
  }
  return permisos;
...
async function listar(propietarioId) {
  const { rows } = await db.query(
    `SELECT p.id, u.email, p.nombre_completo, p.rol, p.activo, p.permisos, p.created_at
...
async function crear({ propietarioId, email, password, nombreCompleto, rol, permisos }) {
  const rolFinal = rol === 'admin' ? 'admin' : 'vendedor';
  const permisosValidados = validarPermisos(permisos);
  ...
  if (permisosValidados.length > 0) {
    await db.query('UPDATE profiles SET permisos = $1 WHERE id = $2', [permisosValidados, usuarioId]);
  }
...
async function actualizarPermisos(propietarioId, id, permisos) {
  const permisosValidados = validarPermisos(permisos);

  const { rows } = await db.query(
    'UPDATE profiles SET permisos = $1 WHERE id = $2 AND propietario_id = $3 RETURNING id, permisos',
    [permisosValidados, id, propietarioId]
  );
  return rows[0] || null;
}
...
module.exports = { listar, crear, actualizarEstado, actualizarPermisos, resetearPassword };