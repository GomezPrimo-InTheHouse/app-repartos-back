...
async function obtenerEstadoCuentaCliente({ propietarioId, clienteId, desde, hasta }) {
  ...
  // guardado a la fecha local (ART) antes de compararlo — mismo fix que ya se aplicó
  // en repartosEjecuciones.service.js para el mismo problema.
  if (desde) { valDespachos.push(desde); condDespachos.push(`(d.fecha AT TIME ZONE 'America/Argentina/Buenos_Aires')::date >= $${valDespachos.length}::date`); }
  if (hasta) { valDespachos.push(hasta); condDespachos.push(`(d.fecha AT TIME ZONE 'America/Argentina/Buenos_Aires')::date <= $${valDespachos.length}::date`); }

  // LEFT JOIN (antes era JOIN): un despacho "solo devolución de envases" no tiene
  // filas en despacho_items, y con INNER JOIN desaparecía del reporte por completo.
  const { rows: despachos } = await db.query(
    `SELECT d.id, d.numero, d.fecha, d.total, d.tipo,
       COALESCE(
         json_agg(
           json_build_object(
  ...
  if (desde) { valPagos.push(desde); condPagos.push(`(fecha AT TIME ZONE 'America/Argentina/Buenos_Aires')::date >= $${valPagos.length}::date`); }
  if (hasta) { valPagos.push(hasta); condPagos.push(`(fecha AT TIME ZONE 'America/Argentina/Buenos_Aires')::date <= $${valPagos.length}::date`); }
